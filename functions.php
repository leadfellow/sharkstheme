...
/**
 * Add mobile visibility field to all ACF blocks
 */
add_filter('acf/load_field_group', function($field_group) {
    // Only add to block field groups
    ...
    if (isset($field_group['fields']) && is_array($field_group['fields'])) {
        ...
        // Create mobile visibility field
        $mobile_field = [
            'key' => $field_group['key'] . '_show_on_mobile',
            'label' => 'Kuva mobiilis',
            'name' => 'show_on_mobile',
            'type' => 'true_false',
        ...
        array_splice($field_group['fields'], $insert_pos, 0, [$mobile_field]);
    }
